export default function Sidebar({ page, setPage, hotelName, onSignOut, alerts }) {
  const nav = [
        ...
        {nav.map(n => (
          <div key={n.id} onClick={() => setPage(n.id)}
            style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'9px 12px',borderRadius:8,cursor:'pointer',marginBottom:2,
              background: page === n.id ? '#1d4ed8' : 'transparent',
              color: page === n.id ? '#fff' : '#94a3b8',
              transition:'.15s',fontSize:'.82rem',fontWeight:500}}>
